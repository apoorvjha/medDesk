...
class Model:
    def __init__(self,vocab_size,embedding_dim,input_length,output_classes,fname=None):
        if fname!=None:
            ...
            ################################  Bidirectional LSTM without sparse categorical cross entropy #################################
            #self.model.add(Bidirectional(LSTM(64,return_sequences=True,activation='relu')))
            #self.model.add(Bidirectional(LSTM(64,return_sequences=True,activation='relu')))
            #self.model.add(Bidirectional(LSTM(64,return_sequences=True,activation='relu')))
            #self.model.add(Bidirectional(LSTM(32,activation='relu')))
            ...
            ################################ LSTM without sparse categorical cross entropy ######################################
